...
import { useRoqPlatformUserProfiles, useComplianceTestCount, useAiModelCount, useDocumentationCount } from 'lib/roq';

import { ComplianceTestListPage as ShowCaseList } from 'pages/compliance-tests';

function DashboardPage() {
  const { session } = useSession();

  const complianceTestStat = useComplianceTestCount();

  const aiModelStat = useAiModelCount();

  const documentationStat = useDocumentationCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Compliance Tests',
      result: complianceTestStat?.data,
      isLoading: complianceTestStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Ai Models',
      result: aiModelStat?.data,
      isLoading: aiModelStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Documentations',
      result: documentationStat?.data,
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Compliance Tests
                </Text>
                <NextLink href={`compliance-tests`} passHref legacyBehavior>
                  <Button
